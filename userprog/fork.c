...
static void copy_body_stack_3(struct task_struct* child_thread,
                              struct task_struct* parent_thread,
                              void* buf_page) {
    ...
    if (vaddr_btmp[idx_byte]) {
      ...
      while (idx_bit < 8) {
        if (BITMAP_MASK << idx_bit & vaddr_btmp[idx_byte]) {
          prog_vaddr = vaddr_start + (idx_bit + idx_byte * 8) * PG_SIZE;
...
static int32_t build_child_stack(struct task_struct* child_thread) {
  ...
  uint32_t* esi_addr_in_thread_stack = (uint32_t*)intr_0_stack - 2;
  uint32_t* edi_addr_in_thread_stack = (uint32_t*)intr_0_stack - 3;
  uint32_t* ebx_addr_in_thread_stack = (uint32_t*)intr_0_stack - 4;
  uint32_t* ebp_addr_in_thread_stack = (uint32_t*)intr_0_stack - 5;

  *ret_addr_in_thread_stack = (uint32_t)intr_exit;

  *ebp_addr_in_thread_stack = *ebx_addr_in_thread_stack =
      *edi_addr_in_thread_stack = *esi_addr_in_thread_stack = 0;

  child_thread->self_kstack = ebp_addr_in_thread_stack;
  return 0;
}
  ...
  while (local_fd < MAX_FILES_OPEN_PER_PROC) {
    ...
    if (global_fd != -1) {
      if (is_pipe(local_fd)) {
        ...
        file_table[global_fd].fd_inode->open_cnts++;
      }
    }
    local_fd++;
  }
...
static int32_t copy_process(struct task_struct* child_thread,
  ...
  copy_body_stack_3(child_thread, parent_thread, buf_page);
