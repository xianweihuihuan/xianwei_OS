...
bool shift_status, ctrl_status, alt_status, caps_lock_status, ext_scancode;

static void intr_keyboard_handler() {
  bool shift_down_last = shift_status;
  bool ctrl_down_last = ctrl_status;
  bool alt_down_last = alt_status;
  bool caps_lcoks_last = caps_lock_status;
  ...
  if (break_code) {
    ...
    if ((scancode < 0x0e) || (scancode == 0x29) || (scancode == 0x1a) ||
      ...
      if (shift_down_last && caps_lcoks_last) {
        shift = false;
      } else if (shift_down_last || caps_lcoks_last) {
        shift = true;
      } else {
        shift = false;
      }
