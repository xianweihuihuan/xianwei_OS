%include "boot.inc"
section loader vstart=LOADER_BASE_ADDR

GDT_BASE:                   ; GDT表起始地址
  dd 0x00000000             ; 空描述符
  dd 0x00000000

CODE_DESC:                  ; 代码段描述符
  dd 0x0000FFFF
  dd DESC_CODE_HIGH4

DATA_STACK_DESC:            ; 数据/栈段描述符
  dd 0x0000FFFF
  dd DESC_DATA_HIGH4

VIDEO_DESC:                 ; 显存段描述符
  dd 0x80000007             ; limit=(0xbffff-0xb8000)/4k=0x7
  dd DESC_VIDEO_HIGH4       ; DPL=0

  GDT_SIZE equ $ - GDT_BASE
  GDT_LIMIT equ GDT_SIZE - 1
  times 60 dq 0             ; 预留60个描述符

  SELECTOR_CODE  equ (0x0001<<3) + TI_GDT + RPL0 ; 代码段选择子
  SELECTOR_DATA  equ (0x0002<<3) + TI_GDT + RPL0 ; 数据段选择子
  SELECTOR_VIDEO equ (0x0003<<3) + TI_GDT + RPL0 ; 显存段选择子

total_mem_bytes dd 0        ; 内存总大小

gdt_ptr dw GDT_LIMIT        ; GDTR结构：界限+基址
        dd GDT_BASE

ards_buf times 244 db 0     ; E820内存信息缓冲区
ards_nr dw 0                ; E820条目数量

loader_start:
  xor ebx, ebx               ; EBX=0，第一次调用
  mov edx, 0x534d4150        ; E820签名
  mov di, ards_buf           ; 缓冲区地址
;利用BIOS中断获取物理内存总量
.e280_mem_get_lopp:
  mov ecx, 20                ; 结构体长度
  int 0x15                   ; 调用BIOS获取内存
  jc .e280_failed_so_try_e801
  add di, cx
  inc word [ards_nr]
  cmp ebx, 0
  jnz .e280_mem_get_lopp

  mov cx, [ards_nr]
  mov ebx, ards_buf
  xor edx, edx               ; 最大内存
.find_max_mem_area:
  mov eax, [ebx]
  add eax, [ebx + 8]
  add ebx, 20
  cmp edx, eax
  jge .next_ards
  mov edx, eax
.next_ards:
  loop .find_max_mem_area
  
  jmp .mem_get_ok

.e280_failed_so_try_e801:
  mov ax, 0xe801
  int 0x15
  jc .e801_failed_so_try_88

  mov cx, 0x400
  mul cx
  shl edx, 16
  and eax, 0x0000FFFF
  or edx, eax
  add edx, 0x100000
  mov esi, edx

  xor eax, eax
  mov ax, bx
  mov ecx, 0x10000
  mul ecx
  add esi, eax

  mov edx, esi
  jmp .mem_get_ok

.e801_failed_so_try_88:
  mov ah, 0x88
  int 0x15
  jc .error_hlt
  and eax, 0x0000FFFF
  mov cx, 0x400
  mul cx
  shl edx, 16
  or edx, eax
  add edx, 0x100000

.mem_get_ok:
  mov [total_mem_bytes], edx   ; 保存内存大小

; --------------- 进入保护模式 ---------------
; 开启A20，加载GDT，启用PE
  in al, 0x92
  or al, 0000_0010b
  out 0x92, al

  lgdt [gdt_ptr]

  mov eax, cr0
  or eax, 0x00000001
  mov cr0, eax

  jmp dword SELECTOR_CODE : p_mode_start;进入32位保护模式，重新加载流水线

;获取物理内存总量失败，CPU停止
.error_hlt:
  hlt

[bits 32]
p_mode_start:
  mov ax, SELECTOR_DATA        ; 初始化段寄存器
  mov ds, ax
  mov es, ax
  mov ss, ax
  mov esp, LOADER_STACK_TOP
  mov ax, SELECTOR_VIDEO
  mov gs, ax

  ; ---------------- 载入内核 ----------------
  mov eax, KERNEL_START_SECTOR
  mov ebx, KERNEL_BIN_BASE_ADDR
  mov ecx, 200
  call rd_disk_m_32

  call setup_page               ; 创建页表

  ; 视频段映射到高端内存
  mov ebx, [gdt_ptr + 2]
  or dword [ebx + 0x18 + 4], 0xc0000000

  add dword [gdt_ptr + 2], 0xc0000000 ; GDT高端映射
  add esp, 0xc0000000                 ; 栈高端映射

  mov eax, PAGE_DIR_TABLE_POS
  mov cr3, eax                  ; 设置页目录

  mov eax, cr0                  ; 开启分页
  or eax, 0x80000000
  mov cr0, eax

  lgdt [gdt_ptr]                ; 重新加载GDT
enter_kernel:
  call kernel_init
  mov esp, 0xc009f000
  jmp KERNEL_ENTRY_POINT

; ---------------- kernel_init: 拷贝内核段 ----------------
;解析内核文件的elf header和pragrom header ，载入内核
kernel_init:
  xor eax, eax
  xor ebx, ebx
  xor ecx, ecx
  xor edx, edx

  mov dx, [KERNEL_BIN_BASE_ADDR + 42] ; e_phentsize
  mov ebx, [KERNEL_BIN_BASE_ADDR + 28] ; e_phoff
  add ebx, KERNEL_BIN_BASE_ADDR
  mov cx, [KERNEL_BIN_BASE_ADDR + 44] ; e_phnum
.each_segment:
  cmp byte [ebx + 0], PT_NULL
  je .PTNULL

  push dword [ebx + 16]      ; size
  mov eax, [ebx + 4]         ; p_offset
  add eax, KERNEL_BIN_BASE_ADDR
  push eax                   ; src
  push dword [ebx + 8]       ; dst
  call mem_cpy
  add esp, 12
.PTNULL:
  add ebx, edx
  loop .each_segment
  ret

; ---------------- mem_cpy: 内存拷贝 ----------------
mem_cpy:
  cld
  push ebp
  mov ebp, esp
  push ecx
  mov edi, [ebp + 8]         ; dst
  mov esi, [ebp + 12]        ; src
  mov ecx, [ebp + 16]        ; size
  rep movsb
  pop ecx
  pop ebp
  ret

; ---------------- setup_page: 创建页目录和页表 ----------------
setup_page:
  mov ecx, 4096
  mov esi, 0
.clear_page_dir:
  mov byte [PAGE_DIR_TABLE_POS + esi], 0
  inc esi
  loop .clear_page_dir

  mov eax, PAGE_DIR_TABLE_POS
  add eax, 0x1000            ; 第一个页表
  mov ebx, eax
  or eax, PG_US_U | PG_RW_W | PG_P
  mov [PAGE_DIR_TABLE_POS + 0x0], eax
  mov [PAGE_DIR_TABLE_POS + 0xc00], eax

  sub eax, 0x1000
  mov [PAGE_DIR_TABLE_POS + 4092], eax

  mov ecx, 256
  mov esi, 0
  mov edx, PG_US_U | PG_RW_W | PG_P
.create_pte:
  mov [ebx + esi*4], edx
  add edx, 4096
  inc esi
  loop .create_pte

  mov eax, PAGE_DIR_TABLE_POS
  add eax, 0x2000
  or eax, PG_US_U | PG_RW_W | PG_P
  mov ebx, PAGE_DIR_TABLE_POS
  mov ecx, 254
  mov esi, 769
.create_kernel_pde:
  mov [ebx + esi*4], eax
  inc esi
  add eax, 0x1000
  loop .create_kernel_pde
  ret

; ---------------- rd_disk_m_32: 从硬盘读取 ----------------
rd_disk_m_32:
  mov esi, eax               ; 扇区号
  mov di, cx                 ; 扇区数

  mov dx, 0x1f2
  mov al, cl
  out dx, al                 ; 写入扇区数

  mov eax, esi
  mov dx, 0x1f3
  out dx, al                 ; LBA低8位

  shr eax, 8
  mov dx, 0x1f4
  out dx, al                 ; LBA 15~8位

  shr eax, 8
  mov dx, 0x1f5
  out dx, al                 ; LBA 23~16位

  shr eax, 8
  and al, 0x0f
  or al, 0xe0
  mov dx, 0x1f6
  out dx, al                 ; LBA 27~24 + 驱动器号

  mov dx, 0x1f7
  mov al, 0x20
  out dx, al                 ; 读扇区命令

.not_ready:
  in al, dx
  and al, 0x88
  cmp al, 0x08
  jnz .not_ready

  mov ax, di
  mov dx, 256
  mul dx
  mov cx, ax
  mov dx, 0x1f0
.go_on_read:
  in ax, dx
  mov [ebx], ax
  add ebx, 2
  loop .go_on_read
  ret
